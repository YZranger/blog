...
.hero { width: 100%; max-width: 700px; margin-bottom: 3rem; }
...
.hero-terminal {
  ...
  box-shadow: 0 0 40px rgba(0, 255, 65, 0.1), 0 0 80px rgba(0, 255, 65, 0.05);
}
...
.terminal-body { padding: 1.5rem 2rem; font-size: 1rem; line-height: 2; }

.line { margin: 0.3rem 0; }
.line.command { color: var(--text-secondary); }
.line.output { color: var(--text-primary); padding-left: 1rem; }

.prompt { color: #ff0055; margin-right: 10px; }

.cursor { display: inline-block; opacity: 0; color: var(--accent); }
.cursor.visible { opacity: 1; animation: blink 0.5s infinite; }

@keyframes blink { 0%, 50% { opacity: 1; } 51%, 100% { opacity: 0; } }

.typing::after { content: '|'; animation: typing 0.5s infinite; color: var(--accent); }
...
.quick-links { display: flex; gap: 1.5rem; }
...
.link-icon { font-family: "Noto Color Emoji", "Apple Color Emoji", "Segoe UI Emoji", sans-serif; }

@media (max-width: 768px) {
  .terminal-body { padding: 1rem 1.5rem; font-size: 0.9rem; }
  .quick-links { flex-direction: column; width: 100%; }
